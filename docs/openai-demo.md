...
2026-09-12 新增 `/openai/` 頁面與右上選單「OpenAI（臨時開放）」，供訪客在 Gemini 額度不足時自行選擇。使用者不需登入、不付費；由站主提供限額的 API 費用。沒有每日／每人訊息數上限，仍受防連點、供應商流量限制與整體預算約束。
...
- 問題及最近對話會送至 OpenAI；頁面已提示勿輸入機密。站方不把對話、名字或 Email 寫入資料庫；`store: false` 不等於供應商完全不保留任何資料。
