...
assert.doesNotMatch(geminiPage, /id="gemini-identity"|gemini-chrome-title|gemini\.connected/);
assert.match(geminiPage, /title\.className = 'gemini-welcome-title'/);
assert.match(geminiPage, /account\.className = 'gemini-welcome-identity'/);
assert.match(geminiPage, /appendWelcome\(session\)/);
assert.match(geminiPage, /classList\.toggle\('gemini-fs-active', fullscreen\)/);
assert.match(geminiPage, /matchMedia\('\(max-width: 640px\)'\)\.matches\) setFullscreen\(true\)/);
